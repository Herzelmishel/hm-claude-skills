...
4. `~/fundraising/story/round-brief.yaml` exists — if missing, stop and refuse with a plain-language message telling the user to run `/preseed-story` first. The pipeline tracks investor state for a specific round; without the round brief there is no round to track.
5. `~/fundraising/pipeline/pipeline.csv` exists — if missing, initialize from schema
6. `~/fundraising/pipeline/touches.csv` exists — if missing, initialize from schema
7. `~/fundraising/pipeline/commitments.csv` exists — if missing, initialize from schema
8. The `investor_id` referenced is present in `~/fundraising/investors/investors.csv`
...
- The soft-circle map (regenerated by `soft_circle_map.py` — descriptor-first; pass `--names-confirmed` only after the user explicitly approves rendering names in the internal table)
- Time-to-close math from `momentum_check.py` (delegated to `/preseed-campaign`)

The soft-circle map is also regenerated automatically after any status change that lands an investor in a `soft_commit` (or stronger) commitment state with `soft_circle_permission = yes`.
...
| `scripts/ghost_check.py` | On every update + every `weekly-review` | Detects 14+ day post-meeting silence (only inbound replies / material engagement reset the timer; founder-initiated touches do not), drafts takeaway |
| `scripts/soft_circle_map.py` | After any soft-commit status change + on `close-check` | Reads `commitments.csv`, regenerates `pipeline/soft-circle-map.md` (descriptor-first; `--names-confirmed` to render the internal Name column) |
